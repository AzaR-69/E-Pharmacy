						...
						<tbody>
						 <c:forEach items="${items}" var="item">
							<tr>
								...
								<td>${item.price}</td>
								<td>${item.quantity}</td>
								<td><a role="button" href="<%=request.getContextPath()%>/views/edititem.jsp?itemId=${item.id}"
										class="btn btn-outline-primary btn-sm ms-3">Edit</a>
									<a role="button" href="<%=request.getContextPath()%>/DeleteDistributorItem?id=${item.id}&itemsId=${item.itemsId}"
